...
class User(Base):
    ...
    vocabulary_entries = relationship(
        "VocabularyEntry", back_populates="user", cascade="all, delete-orphan"
    )
...
class Article(Base):
    ...
    vocabulary_entries = relationship(
        "VocabularyEntry", back_populates="article", cascade="all, delete-orphan"
    )
...
class VocabularyEntry(Base):
    ...
    status = Column(
        String(50), default="learning", nullable=False
    )  # learning, mastered
    created_at = Column(DateTime, default=utc_now, nullable=False)
    updated_at = Column(DateTime, default=utc_now, nullable=False)
...
class Notification(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_id", "type", "source_task_id", name="uq_notifications_user_type_task"
        ),
    )
...
class RecentFeedSource(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_id", "source_url", name="uq_recent_feed_sources_user_url"
        ),
    )
